...
Patches::Patches(float x, float y, float z, color_t color) {
    ...
    GLfloat vertex_buffer_data[2*8*9];
    float ang = M_PI/4.0;
    float curr = ang/2.0;
    int k=0;
    for(int i=1;i<=8;i++){
        vertex_buffer_data[k++] = 3.0*cos(curr);
        vertex_buffer_data[k++] = 3.0*sin(curr);
        ...
        vertex_buffer_data[k++] = 3.0*cos(curr+ang);
        vertex_buffer_data[k++] = 3.0*sin(curr+ang);
        vertex_buffer_data[k++] = 0.0;
        curr+=ang;
    ...
    for(int i=1;i<=8;i++){
        ...
        vertex_buffer_data[k++] = 1.5*cos(curr);
        vertex_buffer_data[k++] = 1.5*sin(curr);
        vertex_buffer_data[k++] = 0.0;
        ...
        vertex_buffer_data[k++] = 1.5*cos(curr+ang);
        vertex_buffer_data[k++] = 1.5*sin(curr+ang);
        vertex_buffer_data[k++] = 0.0;
        curr+=ang;
    }

    this->object = create3DObject(GL_TRIANGLES, 8*3, vertex_buffer_data, color, GL_FILL);
    this->object1 = create3DObject(GL_TRIANGLES, 8*3, vertex_buffer_data+72, COLOR_BLUE, GL_FILL);
}
